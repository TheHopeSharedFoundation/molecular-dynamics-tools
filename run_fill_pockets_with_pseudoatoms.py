...
#/Users/benjaminsamudio/3w32-benzene_NPT_production_2024sep13utc045116_transformed_skip_0_benzenesRemoved.pdb
...
with open("grid_based_fingerprints-SingleFrame.csv","w") as fingerprint_out:
	fingerprint_out.write(f"frame,size,fingerprint\n")
	for frame in range(single_frame,single_frame+1): #<--------------------- changed int(traj.n_frames) to a test integer value
		single_frame = traj[frame]
		single_frame.save('single_frame.pdb')
		protein = scoria.Molecule()
		protein.load_pdb_into("single_frame.pdb")
		#protein_lig = scoria.Molecule()
		#protein_atm = scoria.Molecule()
		indices_lig = protein.select_atoms({'resname':'UNK'})
		indices_atm = protein.invert_selection(indices_lig)
		protein_lig = protein.get_molecule_from_selection(indices_lig)
		protein_atm = protein.get_molecule_from_selection(indices_atm)
		print(f"THIS IS PROTEIN_LIG: {protein_lig}")
		print(f"THIS IS PROTEIN_ATM: {protein_atm}")
		...
		for i in range(0,50):
			...
			#platform_clash_protein = platform.select_close_atoms_from_different_molecules(protein,2.5)
			#platform_clash_pocket = platform.select_close_atoms_from_different_molecules(pocket,3)
			platform_clash_protein = platform.select_close_atoms_from_different_molecules(protein_atm,2.5)
			platform_clash_pocket = platform.select_close_atoms_from_different_molecules(protein_lig,3)
